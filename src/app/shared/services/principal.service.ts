...
/**
 * Service for managing the identity of current User.
 * @class Principal
 */
@Injectable()
export class Principal {
    ...
    hasAnypermissionDirect(permissions: string[]): boolean {
        ...
        for (let i = 0; i < permissions.length; i++) {
            if (this.userIdentity.permissions.includes(permissions[i])) {
                return true;
            }
        }
    ...
    getImageUrl(): String {
        return this.isIdentityResolved() ? this.userIdentity.profilePicture : null;
    }
}
